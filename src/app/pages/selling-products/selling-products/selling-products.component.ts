...
export class SellingProductsComponent extends RootComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
   ...
   if(currentPage != null){
    ...
    this.updateMetaTagSrv.getSeoContent('Best Selling Products').subscribe(
      (data: any) => {
        if (data.meta.status) {
          this.updateMetaTagSrv.updateMetaKeywords(data.data.title,data.data.description,data.data.keywords,"https://toq.co.in/bestsellingproducts"+data.categorySlug,data.data.imageUrl)
        }
      }
    )
